...
def predict(bars: list[Candle], i: int, cfg: DayRangeConfig) -> Prediction:
    ...
    whether it is true.
    """
    if i < MIN_BARS_FOR_A_RANGE:
        return Prediction("none", reason="not enough of the day yet")

    day = bars[max(0, i - 1_440):i + 1]          # the last 24h of bars
    ...
    for i, bar in enumerate(candles):
        ...
        if len(open_trades) < cfg.max_positions and i < len(candles) - 1:
            ...
            if p.direction != "none":
                ...
                need = cfg.lot * oz * bar.close / cfg.leverage
                if used + need <= equity + floating:
                    open_trades.append(Trade(long, entry, cfg.lot, tp, sl,
                                             p.target, i))
            ...
            for edge in (0.20, 0.30, 0.40):
                for confirm in (2, 3):
                    cfg = DayRangeConfig(
                        ...
                        lot=base.lot, max_positions=base.max_positions,
                        leverage=base.leverage,
                        spread_usd_oz=base.spread_usd_oz,
                        edge_fraction=edge, confirm_bars=confirm,
                        min_range_atr=base.min_range_atr,
                        take_fraction=take, stop_fraction=stop,
